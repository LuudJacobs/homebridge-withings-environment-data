...
async function login(email, password, trustCookieName, trustCookieValue) {
  ...
  if (!finalUrl.includes('/new_workflow/exit')) {
    throw new Error(
      `Withings login did not complete (landed on ${finalUrl} instead of /new_workflow/exit) — ` +
        'this can happen if Withings is rate-limiting login attempts, or if the login flow changed.'
    );
  }
